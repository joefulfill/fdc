# FDC System - To Do

## Status: MVP Complete
Core system is in place (FOUNDATION, SCHEDULE, DIET). Now building out tracking and accountability.

---

## Priority 1: Tracking & Accountability System

### Daily Tracking Sheet
- [ ] Create Google Sheet for daily tracking (wife can access)
- [ ] Columns needed:
  - Date
  - Morning weight
  - Sleep (hours + quality 1-10)
  - Meals eaten (Meal 1, 2, 3, Snacks)
  - Calories / Protein tracked (Y/N)
  - Water intake (oz)
  - Adderall (mg + time taken)
  - Training completed (jiu jitsu / strength / cardio)
  - Work hours
  - Family time (hours)
  - Energy level (1-10)
  - Notes / wins / struggles
- [ ] Weekly summary row (average weight, total training sessions, etc.)
- [ ] Share with wife for accountability

### Weekly Meal Plan Tab
- [ ] Pre-planned meals for the week (based on DIET.md options)
- [ ] Grocery list auto-generated from meal plan
- [ ] Wife can see exactly what to prep/cook each day

### Weekly Workout Plan Tab
- [ ] Jiu jitsu sessions (which days/times based on gym schedule)
- [ ] Strength/cardio sessions (which days + what workout)
- [ ] Easy reference for the week ahead

---

## Priority 2: Supporting Documents

### WEEKLY-REVIEW.md
- [ ] Template for Sunday reviews
- [ ] Questions to answer each week
- [ ] Metrics to review
- [ ] Adjustments to make
- [ ] Goals for next week

### TRAINING.md
- [ ] Strength training program (lifts, sets, reps)
- [ ] Cardio programming (type, duration, frequency)
- [ ] Competition prep periodization
- [ ] Home mat session structure

---

## Priority 3: Health Optimization

### Bloodwork & Testing
- [ ] List of markers to test (testosterone, thyroid, metabolic panel, etc.)
- [ ] Find a clinic / order labs
- [ ] Baseline results documented
- [ ] Retest schedule (every 3 months?)

### Supplements & Vitamins
- [ ] Research evidence-based supplements for:
  - Energy/focus (non-stimulant)
  - Recovery
  - Sleep quality
  - General health/longevity
- [ ] Create supplement protocol
- [ ] Add to daily tracking

### Stimulant Protocol
- [ ] Discuss with doctor about sustainable usage
- [ ] Document optimal timing and dosing
- [ ] Track effects on sleep/energy in daily sheet

---

## Priority 4: Financial

### Investment Strategy
- [ ] Schedule conversation with dad
- [ ] Define investment goals and timeline
- [ ] Research options (index funds, real estate, etc.)
- [ ] Create INVESTING.md with strategy
- [ ] Monthly investment tracking

---

## Priority 5: Future Additions

### Competition Tracking
- [ ] Tournament calendar
- [ ] Weight cut protocol (2 weeks out)
- [ ] Competition day routine
- [ ] Results / lessons learned log

### Progress Photos
- [ ] Weekly photos (same day, same lighting)
- [ ] Store in private folder
- [ ] Compare monthly

### Reading List
- [ ] Books to read (Stoicism, business, health)
- [ ] Track books completed
- [ ] Key takeaways documented

---

## Feedback Loop Process

1. **Daily**: Log everything in the tracking sheet
2. **Weekly**: Sunday review - analyze the week, adjust next week's plan
3. **Monthly**: Bigger picture review - weight trend, energy, what's working
4. **With Claude**: Share tracking data, get analysis and adjustments

---

## Next Actions (Immediate)

1. [ ] Create Google Sheet with tracking tabs
2. [ ] Plan this week's meals (Dec 1-7)
3. [ ] Plan this week's workouts (Dec 1-7)
4. [ ] Go grocery shopping (today or tomorrow)
5. [ ] Start Day 1 of the schedule

---

*Last updated: December 1, 2025*
